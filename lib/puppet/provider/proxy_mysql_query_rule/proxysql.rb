...
Puppet::Type.type(:proxy_mysql_query_rule).provide(:proxysql, parent: Puppet::Provider::Proxysql) do
  ...
  def self.instances
    instances = []
    rules = mysql([defaults_file, '-NBre',
  ...
  def flush
    ...
    mysql([defaults_file, '-NBre', 'LOAD MYSQL QUERY RULES TO RUNTIME'].compact) if load_to_runtime == :true

    save_to_disk = @resource[:save_to_disk]
    mysql([defaults_file, '-NBre', 'SAVE MYSQL QUERY RULES TO DISK'].compact) if save_to_disk == :true
  end
